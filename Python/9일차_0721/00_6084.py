...
# 좌우(스테레오) 채널로 저장하면 2배… 5.1채널이면 6배의 저장공간이 필요하고,녹음 시간이 길면 그 만큼 더 많은 저장공간이 필요하다.
...
# 실제로, 일반적인 CD 음질(44.1KHz, 16bit, 스테레오)로 1초 동안 저장하려면44100 * 16 * 2 * 1 bit의 저장공간이 필요한데,
...
# 출력: 필요한 저장 공간을 MB 단위로 바꾸어 출력한다.단, 소수점 첫째 자리까지의 정확도로 출력하고 MB를 공백을 두고 출력한다.
