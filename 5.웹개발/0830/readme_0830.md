...
2. 색상 단위
  ...
  2) RGB색상(background-color: rgb(0, 255, 0);)
    - 16진수 표기법 혹은 함수형 표기법을 사용해서 
      특정 색을 표현하는 방식
  3) HSL색상(background-color: hsl(0, 100%, 50%);)
    - 색상, 채도, 명도를 통해 특정 색을 표현하는 방식
  ※ a는 alpha(투명도)

3. CSS 문서 표현
  - 텍스트
    - 서체(font-family), 서체 스타일(font-stype, font-weight 등)
    - 자간(letter-sapcing), 단어 간격(word-spacing), 
      행간(line-height) 등
...
1. 선택자(Selector) 유형
  ...
  - 의사 클래스/요소(Pseudo Class)
    - 링크, 동적 의사 클래스
    - 구조적 의사클래스, 기타 의사클래스, 의사 엘리먼트, 속성 선택자
...
4.CSS 상속
  - CSS는 상속을 통해 부모 요소의 속성을 자식에게 상속한다.
    ...
    - 상속되는 것 예시
      ex) Text 관련 요소(fonr, color, text-align), 
...
2. Box model
  ...
  - 하나의 박스는 네 부분(영역)으로 이루어짐
    ...
    - border : 테두리 영역
    - padding : 테두리 안쪽 내부 여백, 적용된 배경색/이미지까지 적용
...
2. 대표적으로 활용되는 display
  ...
  - display: none
    ...
    - 이와 비슷한 visibility: hidden; 은
      해당 요소가 공간은 차지하나 화면에 표시만 하지 않는다.
    - 이외 다양한 display 속성은
      https://developer.mozilla.org/ko/docs/Web/CSS/display 참고
